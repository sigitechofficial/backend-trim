...
function scheduleSessions(appointment, sessions) {
  ...
  console.log('🚀 ~ scheduleSessions ~ appointment:', appointment)
  ...
  for (const session of sessions) {
    ...
    console.log(
      '🚀 ~ scheduleSessions ~ currentStartTime + sessionDuration:',
      currentStartTime + sessionDuration * 60 * 1000,
    )
    console.log(
      '🚀 ~ scheduleSessions ~ <= parseTimeString(endTime):',
      parseTimeString(endTime),
    )

    const sessionEndTime = currentStartTime + sessionDuration * 60 * 1000
    sessionSchedule.push({
      startTime: DateManupulation.convertTo24HourFormat(
        formatTime(currentStartTime),
      ),
      endTime: DateManupulation.convertTo24HourFormat(
        formatTime(sessionEndTime),
      ),
...
function assignEmployeesToServices(employeeData, services) {
  ...
  services.forEach((service) => {
    ...
    for (const employee of employeeData) {
      ...
      const providesService = employee.employeeServices.some((empService) =>
        empService && empService.service
          ? empService.service.id === service.serviceId
          : false,
      )
      console.log(
        'ðŸš€ ~providesService:',
      ...
      const isAvailableOnDay =
        ...
        employee.times.some((time) => {
          //
          return (
            new Date(`1970-01-01T${serviceStartTime}`) >=
              new Date(`1970-01-01T${time.openingTime}`) &&
            new Date(`1970-01-01T${serviceEndTime}`) <=
              new Date(`1970-01-01T${time.closingTime}`)
          )
        })
      console.log(
        'ðŸš€ ~ file: customFunctions.js:88 ~ assignEmployeesToServices ~ isAvailableOnDay:',
        isAvailableOnDay,
      )
      ...
      console.log(
        'ðŸš€ ~ Check if the employee is available during the service time',
        isAvailable,
      )
...
const peakTimeReportGenerator = (input) => {
  ...
  console.log(
    '🚀 ~ file: customFunctions.js:205 ~ peakTimeReportGenerator ~ appointments:',
    appointments,
  )
  ...
  appointments.forEach((appointment) => {
    ...
    console.log(
      '🚀🚀🚀🚀🚀🚀 hourlyCountByDay[appointmentDay] = hourlyCountByDay[appointmentDay] || {}:',
      (hourlyCountByDay[appointmentDay] =
        hourlyCountByDay[appointmentDay] || {}),
    )
    hourlyCountByDay[appointmentDay][appointmentHour] =
      (hourlyCountByDay[appointmentDay][appointmentHour] || 0) + 1
    console.log(
      '🚀 ~🚀 ~🚀 ~🚀 ~🚀 ~🚀 ~',
      (hourlyCountByDay[appointmentDay][appointmentHour] =
        (hourlyCountByDay[appointmentDay][appointmentHour] || 0) + 1),
